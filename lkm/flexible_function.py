"""
NOTE: taken from https://github.com/jamesrobertlloyd/gpss-research
...
import numpy as np
from lkm.misc import colored, format_if_possible
...
class FunctionWrapper:
    ...
    def __eq__(self, other):
        if type(self) == type(other):
            ...
            return self.__repr__().__eq__(other.__repr__())

    def __ne__(self, other):
        if type(self) == type(other):
            return False
        else:
            return self.__repr__().__ne__(other.__repr__())
        ...
        if not k.is_operator:
            ...
            for op in ops:
                if isinstance(op, SqExpKernel):
                    if op.dimension not in lengthscales:
    ...
    def initialise_params(self, sd=1, data_shape=None):
        ...
        if self.period == None:
            ...
            if np.random.rand() < 0.33:
                ...
                # else:
                #     self.period = sample_truncated_normal(loc=data_shape['x_sd'][self.dimension]-2, scale=sd, min_value=data_shape['min_period'][self.dimension])
                ...
                # else:
                #     self.period = sample_truncated_normal(loc=np.log(data_shape['x_max'][self.dimension]-data_shape['x_min'][self.dimension])-3.2, scale=sd, min_value=data_shape['min_period'][self.dimension])
                ...
                # else:
                #     self.period = sample_truncated_normal(loc=np.log(data_shape['x_min_abs_diff'][self.dimension]) + 3.2, scale=sd, min_value=data_shape['min_period'][self.dimension])
        ...
        if self.period == None:
            ...
            if np.random.rand() < 0.5:
                if data_shape['min_period'] is None:
                    ...
                    self.period = dff.utils.misc.sample_truncated_normal(loc=data_shape['x_sd'][self.dimension] - 2, scale=sd, min_value=data_shape['min_period'][self.dimension])
            else:
                if data_shape['min_period'] is None:
                    self.period = np.random.normal(loc=np.log(data_shape['x_max'][self.dimension]-data_shape['x_min'][self.dimension])-3.2, scale=sd)
                else:
                    self.period = dff.utils.misc.sample_truncated_normal(loc=np.log(data_shape['x_max'][self.dimension] - data_shape['x_min'][self.dimension]) - 3.2, scale=sd, min_value=data_shape['min_period'][self.dimension])
        if self.sf == None:
            # Set scale factor with output scale or neutrally
            if np.random.rand() < 0.5:
                self.sf = np.random.normal(loc=data_shape['y_sd'], scale=sd)
